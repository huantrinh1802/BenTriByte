...
<article class="w-full rounded-lg border border-gray-200 bg-white p-6 shadow-md @container dark:border-gray-700 dark:bg-gray-800">
  <div class="mb-5 flex flex-col items-center gap-2 text-gray-500 @xs:flex-row">
    <span class="ml-auto justify-self-end text-sm text-primary-700-200-token @xs:hidden">{new Date(post.date).toLocaleDateString()}</span>
    ...
    <span class="ml-auto hidden justify-self-end text-sm text-primary-700-200-token @xs:block">{new Date(post.date).toLocaleDateString()}</span>
  </div>
